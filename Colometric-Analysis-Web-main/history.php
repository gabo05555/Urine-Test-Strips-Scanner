...
<head>
    ...
    <style>
        ...
        /* Suggestion: Move these .btn styles to a shared stylesheet (e.g., styles.css) and import it in all pages to avoid duplication. */
        .hamburger {
            display: none;
            position: absolute;
            top: 15px;
...
<?php require_once 'nav.php'; ?>     
                    ...
                    <thead>
                        <tr>
                            <th>
                                ...
                                <span style="font-size: 0.86em;">Select All</span>
                            </th>
                            <th>Details</th>
                            <th>ID</th>
                            <th>Comment</th>
    ...
    $(document).ready(function () {
        ...
        $('#downloadPdf').on('click', async function () {
            ...
            const headers = ['ID', 'Analysis', 'Image', 'Comment', 'Timestamp'];
            const rows = [];
                ...
                if (image) {
                    try {
                        imgData = await new Promise((resolve, reject) => {
                            ...
                            img.onload = function () {
                                // Increase resolution for the image in the PDF
                                const maxDim = 1080; // high resolution for clarity
                                let w = img.width;
                                let h = img.height;
                                if (w > h && w > maxDim) {
                                ...
                                resolve(canvas.toDataURL('image/png', 1.0)); // use best quality
                            };
            ...
            // Get user name and export date
            const userName = '<?php echo str_replace(" ", "_", htmlspecialchars($_SESSION["name"] ?? "User")); ?>';
            const userNameDisplay = '<?php echo htmlspecialchars($_SESSION["name"] ?? "User"); ?>';
            ...
            doc.autoTable({
                ...
                body: rows.map(row => [
                    ...
                    '', // image cell, will be drawn in didDrawCell
                    row[3],
                    row[4]
                ]),
                startY: 34, // leave more space for the title and header info
                margin: { left: 8 },
                styles: { fontSize: 7, cellPadding: 3, font: 'courier', fontStyle: 'bold' },
                columnStyles: {
                    0: { cellWidth: 28 },
                    1: { cellWidth: 60 },
                    2: { cellWidth: 30, halign: 'center', valign: 'middle' }, // Image column
                    3: { cellWidth: 35, halign: 'left', valign: 'middle' },
                    4: { cellWidth: 40 }
                },
                didParseCell: function (data) {
                    // Remove extra whitespace and indentation in the Comment column (PDF export)
                    if (data.column.index === 3 && data.section === 'body') {
                        if (typeof data.cell.raw === 'string') {
                            data.cell.text = [data.cell.raw.trim().replace(/\s+/g, ' ')];
                ...
                didDrawCell: function (data) {
                    // Draw image in the "Image" column (index 2) for body cells only
                    if (
                        data.section === 'body' &&
                        data.column.index === 2 &&
                        rows[data.row.index] &&
                        ...
                        if (imgData) {
                            // Make the image larger in the cell (e.g., 25x25mm)
                            const cellW = data.cell.width - 2;
                            const cellH = data.cell.height - 2;
                            const maxW = Math.min(60, cellW);
                            const maxH = Math.min(60, cellH);
                            let w = maxW, h = maxH;
                            // Center image in cell
                            const x = data.cell.x + (data.cell.width - w) / 2;
                            const y = data.cell.y + (data.cell.height - h) / 2;
                            try {
                                data.doc.addImage(imgData, 'PNG', x, y, w, h);
                            } catch (e) {
                ...
                didDrawPage: function (data) {
                    ...
                    const title = "Colorimetric Analysis Report";
                    ...
                    const subtitle = "Patient Result List";
                    ...
                    doc.text('Exported by: <?php echo htmlspecialchars($_SESSION["name"] ?? "User"); ?>', data.settings.margin.left, pageHeight - 10);

                    // Bottom right: Exported at
                    const dateText = 'Exported at: ' + exportDate;
                    const textWidth = doc.getTextWidth(dateText);
                    doc.text(dateText, pageWidth - data.settings.margin.right - textWidth, pageHeight - 10);
                }
            });
        ...
        $(document).on('change', '.select_row', function() {
            // If any are unchecked, uncheck master
            if (!$(this).is(':checked')) {
                $('#select_all_rows').prop('checked', false);
            } else if ($('.select_row:checked').length === $('.select_row').length) {
        ...
        // Initially disable Export button if nothing is checked
        toggleExportButton();
    });
